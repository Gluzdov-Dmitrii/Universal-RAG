    ...
    def load(
        ...
        try:
            ...
            if not isinstance(raw_locations, list):
                raise ValueError("cache_locations_invalid")
            locations: list[TextLocation] = []
            for raw_location in raw_locations:
                if not isinstance(raw_location, dict):
                    raise ValueError("cache_location_invalid")
                location = TextLocation(
                    start=int(raw_location["start"]),
                    end=int(raw_location["end"]),
                    kind=str(raw_location["kind"]),
                    value=str(raw_location["value"]),
                )
                if location.end > len(text):
                    raise ValueError("cache_location_out_of_bounds")
