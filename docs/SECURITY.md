...
9. HF-модели по умолчанию открываются только из локального cache; сетевой prefetch —
   отдельный явный setup-режим.
10. Автоматический и ручной ответы проходят одну проверку известных raw-значений до
    demarker; маркированный ответ сохраняется до локального восстановления.
