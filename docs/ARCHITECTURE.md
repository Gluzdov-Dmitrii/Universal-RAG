...
Для retrieval используется отдельное короткое окно только из предыдущих пользовательских
вопросов. Open WebUI background requests помечаются `metadata.task`; они выполняются без
чтения или изменения состояния основного чата, а `follow_up_generation` завершается локально.
...
маленькие chunks остаются поисковыми якорями. Абсолютный путь не является identity документа:
система разрешает сохранённый относительный путь внутри текущего source root, затем повторно
проверяет metadata и SHA-256. Перед privacy boundary лучший документ передаётся целиком, если
он укладывается в `whole_document_max_chars`. Для остальных или крупных источников собирается
ограниченный parent из релевантных pages/slides/sheets; в большой таблице сохраняются заголовок
и область совпадения. Число читаемых источников, полных документов и общий объём ограничены
отдельно. Provider может запросить новые queries для
упомянутых или связанных документов и дополнительный контекст вокруг уже разрешённого
citation. Повторная extraction использует bounded process cache, NER выполняет token windows
пакетами на GPU, а координаты сущностей неизменной ревизии кэшируются без исходных значений.
Реальные абсолютные пути собираются из текущего проверенного source root,
