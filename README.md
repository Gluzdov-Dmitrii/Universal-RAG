...
`scripts/start-local.ps1` загружает локальные настройки из игнорируемого Git файла `.env`.
Если дополнительно задать `SECURE_RAG_CODEX_PERSIST_THREADS=1`, SDK сохраняет отдельную задачу
Codex `Secure RAG <request-prefix>` для каждого запроса: маркированный запрос и ответ можно
проверить в её истории. Без параметра вызовы остаются ephemeral. Одна задача на запрос не
смешивает историю и одинаковые marker ID разных пользователей. Эти задачи отображаются в
разделе Tasks, потому что безопаснее не назначать агенту рабочим каталогом весь RAG-проект.
