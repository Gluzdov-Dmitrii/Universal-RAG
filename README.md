...
- document-first context: лучший источник передаётся целиком в пределах лимита, остальные —
  как page/slide/sheet-aware parent; extraction и NER-позиции кэшируются в памяти процесса;
...
Если локальный корень синхронизации был перенесён, retrieval разрешает сохранённые
относительные пути только внутри нового `SECURE_RAG_SOURCE_ROOT` и заново проверяет metadata и
SHA-256. Следующий index scan постепенно обновляет абсолютные пути без повторных embeddings.
...
Для semantic retrieval к текущей реплике добавляются только несколько последних вопросов
пользователя: это сохраняет ссылку вроде «он» или «этот проект», не размывая embedding
системными инструкциями и длинными ответами. Служебные `metadata.task` запросы Open WebUI не
попадают в пользовательскую историю; автоматические follow-up подсказки для этой модели
возвращаются пустыми без запуска RAG.

Во время потокового ответа модель показывает фактические этапы обработки: проверку запроса,
поиск в Qdrant, количество найденных фрагментов, число собранных документов и сколько из них
передано целиком, sanitization, вызов модели, восстановление ответа и сохранение состояния.
Это операционные статусы, а не скрытые рассуждения модели.
При сбое пользователь видит безопасное название этапа и код диагностики. Соответствующий
JSONL-журнал администратора находится в `runtime/diagnostics/pipeline-events/`; тексты
документов, вопросы и реальные пути в него не записываются.

При `SECURE_RAG_PROVIDER=codex-local` видимые задачи Codex группируются в отдельном
сохранённом проекте. Для этого `SECURE_RAG_AGENT_WORKSPACE_ROOT` должен в точности совпадать с
корнем проекта Codex, а управляемые инструкции сначала разворачиваются туда командой
`scripts/sync-rag-agent-workspace.ps1`. Простого соседства каталогов недостаточно: другой
`cwd` оставляет SDK-задачи в глобальном Recents. Codex назначает `projectId` асинхронно,
обычно после завершения первого turn. Один Open WebUI запрос соответствует одной задаче
Codex; полный контекст чата всё равно передаётся из серверного состояния.
